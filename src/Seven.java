...
/**
 * Created by giuseppeliguori on 08/12/2017.
 */
public class Seven {

    String input = "gbyvdfh (155) -> xqmnq, iyoqt, dimle\n" +
            "oweiea (97)\n" +
            "szhxrs (14)\n" +
            "pjvwsiw (23)\n" +
    ...
    private void partTwo(String input) {
        ...
        (Arrays.asList(input.split("\\n"))).forEach(
                line -> {
                    ...
                    disks.put(values[0].split(" ")[0], Integer.valueOf(values[0].split(" ")[1].replace(" ", "").replace("(", "").replace(")", "")));
                    if (values.length == 2) {
                        elements.put(values[0].split(" ")[0], Arrays.asList(values[1].replace(" ", "").split(",")));
                    } else {
                        elements.put(values[0].split(" ")[0], new ArrayList<>());
                    }
                }
        );

        Iterator iteratorDisks = disks.entrySet().iterator();
        while (iteratorDisks.hasNext()) {
            Map.Entry disk = (Map.Entry) iteratorDisks.next();
            System.out.println("Seven.partTwo: " + disk.getValue() + " " + disk.getKey());
        }

        System.out.println("Seven.partTwo******");
        ...
        while (iterator.hasNext()) {
            ...
            int sum = disks.get(line.getKey());
            System.out.print("Seven.partTwo: " + line.getKey() + " " + disks.get(line.getKey()));
            List<String> values = (List<String>) line.getValue();
            System.out.print(" + ( ");
            for (String value : values) {
                sum += disks.get(value);
                System.out.print(disks.get(value) + " ");
            }
            System.out.print(") ");
            System.out.println(" = " + sum);

            sums.put((String) line.getKey(), sum);
        }

        System.out.println("Seven.partTwo******");


        HashMap<String, List<Integer>> equalChecker = new HashMap<>();
        Iterator iterator1 = sums.entrySet().iterator();
        while (iterator1.hasNext()) {
            Map.Entry val = (Map.Entry) iterator1.next();

            List<Integer> values = new ArrayList<>();
            List<String> strings = elements.get(val.getKey());
            System.out.println("Seven.partTwo: " + val.getKey() + " " + val.getValue());
            for (int i = 0; i < strings.size(); i++) {
                String string = strings.get(i);
                values.add(sums.get(string));


//                if (i == 0) {
//                    startValue = sums.get(string);
//                } else if (startValue != sums.get(string)){
////                    System.out.println("Seven.partTwo: OHOHOHOH");
//                    System.out.println("Seven.partTwo:" + val.getKey() + " " + val.getValue());
//
//                    break;
//                }
            ...
            if (values.size() != 0) {
                ...
                equalChecker.put((String) val.getKey(), values);
            }
        }
        ...
        Iterator iterator2 = equalChecker.entrySet().iterator();
        while (iterator2.hasNext()) {
            Map.Entry value = (Map.Entry) iterator2.next();
            String string = (String) value.getKey();
            List<Integer> list = (List<Integer>) value.getValue();

            System.out.print("Seven.partTwo: " + string);
            int prev = -1;
            int wrongsCounter = 0;
...
//exbktzw
    ...
    private void partOne(String input) {
        ...
        while (iterator.hasNext()) {
            ...
            if (repetitions.containsKey(value)) {
                repetitions.replace(value, repetitions.get(value) + 1);
            } else {
            ...
            for (String value2 : values) {
                if (repetitions.containsKey(value2)) {
                    repetitions.replace(value2, repetitions.get(value2) + 1);
                } else {
                    repetitions.put(value2, 1);
                }
            }
        }

        Iterator iteratorRepetitions = repetitions.entrySet().iterator();
        while (iteratorRepetitions.hasNext()) {
            Map.Entry repetition = (Map.Entry) iteratorRepetitions.next();
            if (1 == (Integer) repetition.getValue()) {
                System.out.println("Seven.partOne: " + repetition.getKey());
            }
        }
