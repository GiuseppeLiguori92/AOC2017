...
//    String input = "n,n,n,n,ne,ne";
    ...
    public void partOne(String input) {
        ...
        matrix[initR][initC] = "X";

        (Arrays.asList(input.split(","))).forEach(instruction ->
                {
                    if (instruction.equals("ne")) {
                        selectedR--;
                        selectedC++;
                    } else if (instruction.equals("n")) {
                        selectedR--;
                        selectedR--;
                    } else if (instruction.equals("nw")) {
                        selectedR--;
                        selectedC--;
                    }

                    if (instruction.equals("se")) {
                        selectedR++;
                        selectedC++;
                    ...
                    matrix[selectedR][selectedC] = "X";
                    printMatrix(matrix, rows, cols);
                }
        );

        int step = 0;
        int diffR = Math.abs(selectedR - initR);
        int diffC = Math.abs(selectedC - initC);
        if (diffR == diffC) {
            step = Math.abs(selectedR - initR);
        } else {
            System.out.println("Eleven.partOne: " + diffR + " " + diffC);
            step = (diffR - diffC)/2 + diffC;
        }
        System.out.println("Eleven.partOne: " + step);
    }
    ...
    private void printMatrix(String[][] matrix, int rows, int cols) {
//        for (int i = 0; i < rows; i++) {
//            for (int j = 0; j < cols; j++) {
...
//        }
    ...
    public void partTwo(String input) {
        ...
        (Arrays.asList(input.split(","))).forEach(instruction ->
                {
                    ...
                    if (diffR == diffC) {
                        ...
                        System.out.println("Eleven.partTwo: " + diffR + " " + diffC);
                        step = (diffR - diffC)/2 + diffC;
                    }
                    System.out.println("Eleven.partTwo: " + step);
        ...
        System.out.println("Eleven.partTwo: " + maxStep);
