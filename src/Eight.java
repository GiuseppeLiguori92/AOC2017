...
public class Eight {
    ...
    String input = "uz inc 134 if hx > -10\n" +
            "qin dec -300 if h <= 1\n" +
...
//        partOne(input);
    ...
    public static class Command {
        ...
        public String getConditionStartRegister() {
            return conditionStartRegister;
        }

        public void setConditionStartRegister(String conditionStartRegister) {
            this.conditionStartRegister = conditionStartRegister;
        }
    ...
    public void partOne(String input) {
        ...
        (Arrays.asList(input.split("\n"))).forEach(line -> {
            ...
            command.setStartRegister(values[0].replace(" ", ""));

            Command.Function function = null;
            switch (values[1].replace(" ", "")) {
                case "inc":
                    function = Command.Function.inc;
                    break;
                case "dec":
                    function = Command.Function.dec;
                    break;
            }
            command.setFunction(function);

            command.setBy(Integer.valueOf(values[2].replace(" ", "")));

            command.setConditionStartRegister(values[4].replace(" ", ""));

            Command.Condition condition = null;
            String conditionString = values[5].replace(" ", "");
            if (conditionString.equals(">"))
                condition = Command.Condition.greather;
            else if (conditionString.equals("<"))
                condition = Command.Condition.lower;
            else if (conditionString.equals("=="))
                condition = Command.Condition.equal;
            else if (conditionString.equals("<="))
                condition = Command.Condition.lowerequal;
            else if (conditionString.equals(">="))
                condition = Command.Condition.greatherequal;
            else if (conditionString.equals("!="))
                condition = Command.Condition.notequal;

            command.setCondition(condition);

            command.setConditionValue(Integer.valueOf(values[6].replace(" ", "")));
            commands.add(command);

        });

        commands.forEach(command -> {
            if (registers.get(command.getStartRegister()) == null) {
                registers.put(command.getStartRegister(), 0);
            }

            if (registers.get(command.getConditionStartRegister()) == null) {
                registers.put(command.getConditionStartRegister(), 0);
            }

            boolean conditionPersist = false;
            switch (command.getCondition()) {
                case greather:
                    conditionPersist = registers.get(command.getConditionStartRegister()) > command.getConditionValue();
                    break;
                case lower:
                    conditionPersist = registers.get(command.getConditionStartRegister()) < command.getConditionValue();
                    break;
            ...
            if (conditionPersist) {
                switch (command.getFunction()) {
                    case inc:
                        registers.replace(command.getStartRegister(), registers.get(command.getStartRegister()) + command.getBy());
                        break;
                    case dec:
                        registers.replace(command.getStartRegister(), registers.get(command.getStartRegister()) - command.getBy());
                        break;
        ...
        Iterator iterator = registers.entrySet().iterator();
        ...
        System.out.println("Eight.partOne: " + max);
    ...
    public void partTwo(String input) {
        ...
        commands.forEach(command -> {
            ...
            if (conditionPersist) {
                ...
                switch (command.getFunction()) {
                    case inc:
                        value = registers.get(command.getStartRegister()) + command.getBy();
                        break;
                    case dec:
                        value = registers.get(command.getStartRegister()) - command.getBy();
                        break;
                }
                registers.replace(command.getStartRegister(), value);
