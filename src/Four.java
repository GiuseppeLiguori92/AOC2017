...
/**
 * Created by giuseppeliguori on 07/12/2017.
 */
public class Four {
...
//        partOne(input);
    ...
    private void partOne(String input) {
        (Arrays.asList(input.split("\\n"))).forEach(line ->
                {
                    (Arrays.asList(line.split(" "))).forEach(
                            word -> words.add(word)
                    );

                    boolean valid = true;
                    HashMap<String, Integer> repeatitions = new HashMap<>();
                    for (String word : words) {
                        if (repeatitions.containsKey(word)) {
                            valid = false;
                            break;
                        }
                        repeatitions.put(word, 1);
                    }
        ...
        System.out.println("Four.partOne: " + validPhrases);
    }
