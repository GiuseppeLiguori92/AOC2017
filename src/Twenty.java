...
/**
 * Created by giuseppeliguori on 20/12/2017.
 */
public class Twenty {
    String inputTest = "p=< 3,0,0>, v=< 2,0,0>, a=<-1,0,0>\np=< 4,0,0>, v=< 0,0,0>, a=<-2,0,0>\np=< 0,0,0>, v=< 0,0,0>, a=<0,0,0>";
    String inputTest2 = "p=<-6,0,0>, v=< 3,0,0>, a=< 0,0,0>\n" +
            "p=<-4,0,0>, v=< 2,0,0>, a=< 0,0,0>\n" +
            "p=<-2,0,0>, v=< 1,0,0>, a=< 0,0,0>\n" +
            "p=< 3,0,0>, v=<-1,0,0>, a=< 0,0,0>";

    String input = "p=<-3787,-3683,3352>, v=<41,-25,-124>, a=<5,9,1>\n" +
...
//        partOne(input);
    ...
    public void partOne(String input) {
        ...
        Arrays.asList(input.split("\\n")).forEach(line -> {
            ...
            line = line.replace(">", "");
            line = line.replace(" ", "");
            Integer[] particle = new Integer[10];
            final int[] i = {0};
            Arrays.asList(line.split(",")).forEach(value -> {
//                System.out.println("input = [" + value + "]");
                particle[i[0]] = Integer.valueOf(value);
                i[0]++;
            });
        ...
        int closeToZero = 0;
        int indexCloseToZero = 0;
        int i = 0;
        while (true) {
            for (int j = 0; j < particles.size(); j++) {
                Integer[] particle = particles.get(j);
                ...
                particle[9] = Math.abs(particle[0]) + Math.abs(particle[1]) + Math.abs(particle[2]);

                System.out.println("Twenty.partOne: [" + j + "] " + particle[9]);

                if (j == 0 || particle[9] < closeToZero) {
                    closeToZero = particle[9];
                    indexCloseToZero = j;
                }
            }

            closest.add(indexCloseToZero);
            if (i > 1000) {
                boolean isSame = true;
                int compareTo = closest.get(closest.size() - 1);
                for (int k = closest.size() - 1; k > closest.size() - 500; k--) {
                    if (closest.get(k) != compareTo) {
                        isSame = false;
                        break;
                    }
                }
                ...
                if (isSame) {
                    System.out.println("Twenty.partOne: " + compareTo);
                    break;
                }
            }
            i++;
    ...
    public void partTwo(String input) {
        ...
        Arrays.asList(input.split("\\n")).forEach(line -> {
            ...
            line = line.replace(">", "");
            line = line.replace(" ", "");
            Integer[] particle = new Integer[11];
            final int[] i = {0};
            Arrays.asList(line.split(",")).forEach(value -> {
//                System.out.println("input = [" + value + "]");
                particle[i[0]] = Integer.valueOf(value);
        ...
        while (true) {
            ...
            for (int j = 0; j < particles.size(); j++) {
                ...
                for (int k = 0; k < particles.size(); k++) {
                    if (particles.get(k)[10] == -1 || j == k) {
                        continue;
                    }

                    if (x == particles.get(k)[0] &&
                            y == particles.get(k)[1] &&
                            z == particles.get(k)[2]) {
                        suicide = true;
                        particles.get(k)[10] = -1;
                    }
                }

                if (suicide) {
                    suicide = false;
                    particles.get(j)[10] = -1;
                }
            }
            ...
            if (i > 1000) {
                ...
                System.out.println("Twenty.partTwo: " + left);
                break;
            }
        }
    }
